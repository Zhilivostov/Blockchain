...
class Blockchain:
    ...
    def register_node(self, address):
        ...
        if parsed_url.netloc:
            ...
            # Принимает URL-адрес без схемы, такой как '192.168.0.5:5000'.
    ...
    def valid_chain(self, chain):
        """
        Определяем, является ли данный блокчейн действительным
        :param chain: Блокчейн
        :return: True, если допустимо, False, если нет
        """
        ...
        while current_index < len(chain):
            ...
            print(f'{block}')
            print("\n-----------\n")
            # Проверка правильности хэша блока
            last_block_hash = self.hash(last_block)
            if block['previous_hash'] != last_block_hash:
                return False

            # Проверяем правильность подтверждения работы
    ...
    def resolve_conflicts(self):
        """
        Это наш алгоритм консенсуса, он разрешает конфликты, заменяя нашу цепочку самой длинной в сети.
        :return: True, если наша цепочка была заменена, False, если нет
        """

        neighbours = self.nodes
        new_chain = None

        # Мы ищем только цепи длиннее наших
        max_length = len(self.chain)

        # Захватываем и проверяем цепочки со всех узлов нашей сети
        for node in neighbours:
            ...
            if response.status_code == 200:
                ...
                # Проверяем, больше ли длина и действительна ли цепочка
                if length > max_length and self.valid_chain(chain):
                    max_length = length
                    new_chain = chain

        # Заменяем нашу цепочку, если мы обнаружили новую, действительную цепочку длиннее нашей
        if new_chain:
...
"""
Это макро-фреймворк, который заметно упрощает сопоставление конечных 
точек с функциями Python. Это позволяет нам взаимодействовать в интернете
 при помощи HTTP-запросов.
 """

# Генерируем уникальный на глобальном уровне адрес для этого узла
node_identifier = str(uuid4()).replace('-', '')
# т е создание случайного имени узла;
...
def mine():
    ...
    # Мы должны получить награду за то, что нашли доказательство.

    # Отправитель равен "0", что означает, что этот узел добыл новую монету.
    ...
    # Создаем новый блок, добавив его в цепочку
...
# Создание конечной точки /transactions/new, которая является POST-запросом,
# так как мы будем отправлять туда данные;
@app.route('/transactions/new', methods=['POST'])
def new_transaction():
    values = request.get_json()

    # Проверяем наличие обязательных полей в POST'ed данных
